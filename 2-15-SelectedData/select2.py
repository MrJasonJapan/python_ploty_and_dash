...
# Note, you need to use the "Box Select" or "Lasso Select" icons above the graph to select points.
@app.callback(
    Output('density', 'children'),
    [Input('plot', 'selectedData')])
def find_density(selectedData):
    # Calculate the density of the selected points
    num_of_pts = len(selectedData['points'])
    rng_or_lp = list(selectedData.keys())  # rng_or_lp stands for "range or lasso points"
    # we don't need the points to caculate the area of the selection.
    # Note that the area calculation of a lasso selection will be rough compared to a box select area.
